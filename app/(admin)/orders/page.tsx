...
const Orders = ({ searchParams }: SearchParamProps) => {
  ...
  const convertToCSV = (array: IOrderItem[]) => {
    const headers = ['Order ID', 'Event Title', 'Buyer', 'Email', 'Created', 'Amount']
    const rows = array.map(order => [
      order._id,
      order.eventTitle,
      order.buyer,
      order.email,
  ...
  return (
    <>
      ...
      <section className="wrapper mt-8">
        ...
        <button onClick={downloadCSV} className="mt-4 px-4 py-2 bg-primary button rounded-full text-white ">
          Download CSV
        </button>
      </section>
